...
class GroupService:
    ...
    async def add_users_to_group(self, group_id: int, data: AddUsersToGroup, current_user: User) -> GroupReadWithRelations:
        ...
        try:
            ...
            group_exists_result = await self.session.execute(group_exists_stmt)
    ...
    async def change_user_role(self, current_user_id: int, group_id: int, user_email: str, new_role: UserRole):
        ...
        try:
            ...
            old_role = group_member.role
            group_member.role = new_role
            await self.session.commit()
            
            self.logger.info(f"Role for user {user_email} changed from {old_role.value} to {new_role.value}")
    ...
    async def remove_users_from_group(self, group_id: int, data: RemoveUsersFromGroup, current_user: User) -> GroupReadWithRelations:
        ...
        except (GroupNotFoundError, InsufficientPermissionsError, UserNotFoundInGroupError, GroupDeleteError):
            raise
        except Exception as e:
            await self.session.rollback()

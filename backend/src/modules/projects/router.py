...
async def create_new_project(project_data: ProjectCreate, session: AsyncSession = Depends(db_session.session_getter)):
    try:
...
async def add_groups_to_project_route(
    ...
    try:
        updated_project = await projects_service.add_groups_to_project(session, project_id, data)
...
async def remove_groups_from_project_route(
    ...
    try:
        updated_project = await projects_service.remove_groups_from_project(session, project_id, data)
        return updated_project
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))

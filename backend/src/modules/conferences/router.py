...
async def build_room_details(conference_service, current_user: User, room) -> ConferenceRoomWithDetails:
...
@router.get("/rooms/invitable-users", response_model=List[InvitableUserResponse])
async def get_invitable_users(
    query: Optional[str] = Query(None, description="Поиск по имени, логину или email"),
...
async def get_available_rooms(
    status_filter: str = Query("active", alias="status", pattern="^(active|ended|all)$"),
    service_factory: ServiceFactory = Depends(get_service_factory),
    current_user: User = Depends(get_current_user),
):
    ...
    conference_service = service_factory.get('conference')
    rooms = await conference_service.get_available_rooms_for_user(current_user.id, status=status_filter)
    return [await build_room_details(conference_service, current_user, room) for room in rooms]
...
async def get_task_conferences(
    ...
    conference_service = service_factory.get('conference')
    return await conference_service.get_rooms_by_task(task_id, current_user.id, status=status_filter)
...
async def get_room_details(
    ...
    return await build_room_details(conference_service, current_user, room)
...
@router.get("/rooms/{room_id}/leave-impact", response_model=LeaveConferenceImpactResponse)
...
async def leave_conference_room(
    room_id: int,
    leave_data: LeaveConferenceRequest = Body(default_factory=LeaveConferenceRequest),
    service_factory: ServiceFactory = Depends(get_service_factory),
    current_user: User = Depends(get_current_user),
):
    ...
    conference_service = service_factory.get('conference')
    success = await conference_service.leave_room(
        room_id,
        current_user.id,
        auto_end_if_last=leave_data.auto_end_if_last,
    )
